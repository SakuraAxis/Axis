...
function _close_axis_rs!()
    empty!(_AXIS_RS_SYMBOLS)
    if _AXIS_RS_HANDLE[] != C_NULL
        try
            Libdl.dlclose(_AXIS_RS_HANDLE[])
        catch
        end
...
#=
bridge_up — automate generate_bridge + cargo build --release
=#

"""
    bridge_up(axis_rs_path::String)

Generate the Rust bridge files in `axis_rs_path/src/axis_generated` and compile the
Rust crate with `cargo build --release`. After the build, the Rust library is
reloaded automatically so you can use your `@rust_fn` functions immediately.
"""
function bridge_up(axis_rs_path::String)
    isdir(axis_rs_path) ||
        error("bridge_up: axis_rs directory not found at `$axis_rs_path`.")

    generated_dir = joinpath(axis_rs_path, "src", "axis_generated")

    @info "Axis.bridge_up: generating Rust bridge..." generated_dir
    generate_bridge(generated_dir)
    ...
    @info "Axis.bridge_up: reloading shared library..."
    _init_axis_rs!()

    @info "Axis.bridge_up: done! " *
          "You can now call your @rust_fn functions via @AX.call_rust_fn."
    return nothing
end

#=
bridge_down — clean/teardown generated files in axis_rs
=#

"""
    bridge_down(axis_rs_path::String)

Teardown the generated bridge. This unloads the Rust dynamic library handle,
deletes all files in `axis_rs_path/src/axis_generated/`, and leaves a blank `mod.rs`
so that subsequent Rust compilations do not fail.
"""
function bridge_down(axis_rs_path::String)
    isdir(axis_rs_path) ||
        error("bridge_down: axis_rs directory not found at `$axis_rs_path`.")

    @info "Axis.bridge_down: unloading shared library handle..."
    _close_axis_rs!()

    generated_dir = joinpath(axis_rs_path, "src", "axis_generated")
    if isdir(generated_dir)
        @info "Axis.bridge_down: cleaning generated directory..." generated_dir
        for f in readdir(generated_dir; join=true)
            if isfile(f)
                rm(f)
            elseif isdir(f)
                rm(f; recursive=true)
    ...
    # Write a blank mod.rs so cargo build still compiles
    mod_rs_path = joinpath(generated_dir, "mod.rs")
    open(mod_rs_path, "w") do io
        println(io, "// AUTO-GENERATED by Axis.bridge_down() — DO NOT EDIT.")
        println(io, "// Bridge has been dismantled.")
    end

    @info "Axis.bridge_down: teardown complete."
    return nothing
end
